...
class headerLogo extends Component {
    render() {
        ...
        const { icon, title, back, image, noLogo, onBackPress, onImagePress, drawer, nomap,noAdd,onImageClick,goBack,search,placeholderText,searchValue,fnc,onPressFilter,filter } = this.props
        ...
        return (
            <KeyboardAvoidingView style={{flexGrow: 1, height: '100%', }} behavior={Platform.OS=='ios'?'padding':'height'} enabled keyboardVerticalOffset={Platform.OS=='android'?80:20} >
                <ScrollView bounces={false} ref={refs=> this.scroll = refs} keyboardShouldPersistTaps='handled' contentContainerStyle={styles.scroll} >
                    ...
                    <ImageBackground source={Images.header} style={{width:'100%', height:dp(170), alignItems:'center', paddingTop:dp(30), marginBottom:dp(40)}}  >
                        <View style={{
                            ...
                            {back || drawer?
                                <TouchableOpacity style={!nomap?styles.backButton: null} onPress={()=>{
                                    if(drawer) 
                        ...
                        </View>
                        {
                            ...
                            <Image style={{width:dp(15),height:dp(15),margin:dp(15)}} source={Images.searchicon} />
                            <TextInput 
                        value={searchValue}
                        onChangeText={(text)=>{
                            fnc(text)
                ...
                {noAdd?
                <TouchableOpacity style={{alignSelf:'flex-end', position:'absolute', bottom:10,right:15}} onPress={()=>onImageClick()} >
                <Image source={Images.addPlus} />
            </TouchableOpacity>
            : null
        }
             {filter?
                <TouchableOpacity style={{alignSelf:'flex-end', position:'absolute', bottom:10,right:15}} onPress={()=>onPressFilter()} >
