...
export class EventDetail extends Component {
    ...
    componentDidMount=async()=> {
          ...
          Participant=async()=>{
              // alert('You have paricipate this event')

              // console.log("idssss====>>>>",message)
                              const userId = await AsyncStorage.getItem('userId');
                              console.log("userIddddddddddddddddd====>", userId)
                              console.log("idssss====>>>>",userId, "eventId====>", this.state.event.id)
    ...
    render() {
        return (
            ...
            <Header title={'EVENT DETAILS'} search back goBack={() => this.props.navigation.goBack()}  >
                <View style={Styles.maincontainer}>
                    <View style={Styles.innercontainer}>
                        <Text style={Styles.heading}>{this.state.event.name}</Text>
                        {
                            this.state.event.image ?
                                <Image source={this.state.event.image ? { uri: this.state.event.image } : null} style={Styles.profilePic} />
                                : null
                        }
                        {
                            this.props.navigation.state.params.request ?
                                <View style={{ flexDirection: 'row', marginTop: dp(8), }}>
                                    ...
                                    <Text style={{ color: '#0D3447', fontSize: dp(12), fontWeight: '500', }}>{this.state.event.organiser}</Text>
                                </View>
                                : null
                        }
                        ...
                        <View style={[Styles.commonSpace, Styles.wrap]}>
                            ...
                            <Text style={Styles.des}>{strings.age}: {this.state.event.age_range}</Text>
                        </View>
                        ...
                        <Text style={Styles.des}>{this.state.event.start_date} - {this.state.event.end_date}</Text>
                        <Text style={Styles.des}>{this.state.event.start_time} - {this.state.event.end_time}</Text>

                        <Text style={Styles.boldTheme}>0/{this.state.event.total_participants} Participants needed</Text>
...

                        {
                            this.state.event.user_type == 1 ?
                                <View>
                                    ...
                                    <Text style={Styles.des}>{this.state.event.recurrence} Times</Text>
                                </View> : null
                        }
                        ...
                        <View style={{
                            ...
                            >
                                 <Marker
                                        coordinate={{latitude: Number(this.state.event.lat) , longitude: Number(this.state.event.lng)}}
                        ...
                        {
                            this.props.navigation.state.params.request && !this.state.reqsted?  
                                <Button label={strings.join} bold  onPress={this.Participant} /> :  null
                                       
                        }
                          { this.state.reqsted ?
                                <Button label={strings.requested} bold disabled/> : null }

                        {
                            this.props.navigation.state.params.interest ?
                                <View>
                                    <Button label='INTERESTED PARTIPANTS' onPress={() =>{
                                                this.props.navigation.navigate('Participant', {config:this.state.event.id})}} />
                                                         <Button label='LEAVE GROUP' onPress={() => null} buttonStyle={{ backgroundColor: '#FFF', borderColor: EStyleSheet.value('$theme2'), borderWidth: 1 }} color={EStyleSheet.value('$theme')} />
                                </View> : null
                        }
...

                        {
                            this.props.navigation.state.params.view ?
                                <View>
                                    <Button label='VIEW PARTICIPANTS' onPress={() =>{
                                                        this.props.navigation.navigate('Participant', {config:this.state.event.id})}} />
