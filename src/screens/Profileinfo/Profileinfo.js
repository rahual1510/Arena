...
export class Profileinfo extends Component {
    constructor(props) {
        ...
        this.state.info = this.props.navigation.state.params.Profileinfo
        this.state.age = parseInt(new Date().getFullYear()) - parseInt(new Date(this.props.navigation.state.params.Profileinfo.date_birth).getFullYear());
    ...
    async sendMessage() {
        ...
        if (userId) {
            ...
            let receiverName = this.state.info.first_name.toUpperCase() + (this.state.info.last_name ? +" " + this.state.info.last_name.toUpperCase() : null)
            let data = {
                receiverName,
                    ...
                    <ScrollView bounces={false} ref={refs => this.scroll = refs} keyboardShouldPersistTaps='handled' contentContainerStyle={styles.scroll} >
                        <View style={{ flex: 1, padding: 20 }}>
                            ...
                            <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: '15%', marginBottom: -20 }}>
                                ...
                                <Image source={this.state.info.image ? { uri: this.state.info.image } : Images.dummyPic} style={styles.profilePic} />
                                ...
                                <View style={styles.outerBox}>
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <Text style={styles.text}>
                                            {this.state.info.first_name.toUpperCase()} {this.state.info.last_name ? this.state.info.last_name.toUpperCase() : null}
                                        </Text>
                                        <Text style={styles.dulltext}>
                                            Age : {2020 - parseInt(this.state.info.date_birth.split('-')[2])}
                                        </Text>
                            ...
                            <Button label={strings.message} bold onPress={() => this.sendMessage()} />
