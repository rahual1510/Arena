...
export class FindEvent extends Component {
    ...
    render() {
        return (
            <Header onMapPress={() => this.props.navigation.navigate('Mapview', { atheletList: this.props.athleteList })}
                title="Find ATHLETES" noLogo={true} nomap={true} filter={true} onPressFilter={() => ''} back onBackPress={() => this.props.navigation.goBack()} search placeholderText={'Search'} fnc={(sval) => this.filter(sval)}  >
